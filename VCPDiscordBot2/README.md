...
当 @ 提及或重要频道消息触发 Agent 时，插件会在 Agent prompt 中直接注入：

- 当前频道最近 20 条 Discord 消息。
- 每条消息的时间、作者、消息 ID 和正文。
- 用户消息与 Bot 自己之前发送/回复的消息。
- 当前触发消息的 ID，要求 Agent 回复时调用 `reply_message`。

插件会优先通过 Discord API 读取频道历史；API 暂时失败时使用本地频道缓存回退。上下文总长度有上限，避免过长历史占用模型上下文。

### 回复旧消息时的额外处理

如果用户使用 Discord 原生“回复”功能发送消息，并且被回复消息早于最近 20 条，插件会读取当前消息的 `reference.messageId`，通过 Discord API 单独获取被引用消息，并在 prompt 中加入：
...
因此，原生回复的旧消息不会因为超出最近 20 条而丢失。仅在正文中提到某条旧消息、消息 ID 或历史内容，但没有 Discord 原生引用关系时，插件不会自动推断并读取该消息；这类行为需要额外实现文本解析规则。

示例：

```text
【当前频道最近 20 条消息】
1. [时间] 用户 tester (messageId=111): 你好
2. [时间] Bot VCPBot (messageId=112): 你好，我在
3. [时间] 用户 tester (messageId=113): 请继续刚才的话题
```

### 动态占位符

- `{{VCPDiscordRecentMessages}}`：跨频道的待处理消息面板，用于手动查看消息 ID。
- `{{VCPDiscordBotStatus}}`：Gateway、VCP 配置注入、Agent 投递链路和消息统计状态。
