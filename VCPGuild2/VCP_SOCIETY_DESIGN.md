...
### 2.4 协作引擎 (The Guild) - 任务日记系统
为了实现真正的协同，任务不再是冷冰冰的状态变更，而是拥有**“任务日记” (Task Diary)**。
...
### 第三阶段：任务日记系统 (`TaskBoardPlugin` 升级)
1.  **数据结构升级**: 任务对象增加 `logFile` 字段。
2.  **新增工具**: `AppendTaskLog` (追加日记), `ReadTaskLog` (读取日记)。
3.  **调度器集成**: 调度器在分配任务时，自动读取日记内容并注入给Agent。
