...
调度器只负责发现、匹配和启动，不再持有长任务循环。任务匹配成功后，它先通过 TaskBoard 原子接取任务，再调用 `BackendFlowlockPlugin.StartTaskFlowlock` 建立后端自主会话。若启动失败，调度器使用本次 `assignment_id` 补偿释放任务。
...
5. 调用 TaskBoardPlugin.ListTasks
 ...
 8. TaskBoard 原子接取并生成 assignment_id
            │
            ▼
 9. BackendFlowlockPlugin 建立 taskId 独立会话
            │
            ▼
10. 每轮通过 AgentAssistant 执行，直到提交/失败/停止
```
...
### 3. BackendFlowlockPlugin (后端心流锁服务)

**类型**: 混合服务插件 (`hybridservice`)
**协议**: direct
**位置**: `Plugin/BackendFlowlockPlugin/`

该插件把前端 Flowlock 的核心状态机语义扩展到 VCP 后端，但不依赖前端运行：

- 会话键为 `agentName + taskId`，同一 Agent 可并行处理多个任务。
- 每个任务使用独立 AgentAssistant `session_id`、定时器、generation、轮次和重试状态。
- 每轮开始前读取结构化任务状态与最近任务日记。
- 支持 Start、Stop、Complete、Fail、NextHeartbeat 和 NextPrompt。
- Session 原子写入 `data/sessions.json`，服务重启后校验任务所有权并恢复。
- `submitted/completed` 任务自动完成会话；释放、超时或负责人变化会停止会话。
- TaskBoard 的状态是完成真源。模型仅输出 Complete 但未调用 SubmitTask 时，会话不会停止。

前端 Flowlock 与 BackendFlowlockPlugin 共享协议语义，但运行态完全独立：前者绑定 Agent/Topic，后者绑定 Agent/Task。
...
2. **任务上限**：每个Agent同时最多接取3个任务，对应最多3个独立后端 Flowlock Session
3. **超时机制**：任务超过24小时未完成自动释放，后端会话下次对账时停止
4. **失败重试**：单会话达到重试上限后停止，不影响同 Agent 的其他任务
5. **原子认领**：TaskBoard 使用跨进程写锁及 `assignment_id` 防止重复接取与错误补偿
6. **防复活**：每个定时器捕获 Session generation，停止或恢复后旧回调失效
7. **恢复校验**：重启后只恢复仍为 `in_progress` 且负责人和分配令牌一致的任务
