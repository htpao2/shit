...
async function tryAssignTask(task) {
    ...
    try {
        ...
        const flowlockResult = await pluginManagerRef.processToolCall('BackendFlowlockPlugin', {
        ...
        if (flowlockResult && flowlockResult.status === 'success') {
            if (DEBUG_MODE) {
                console.error(`[AutonomousSchedulerPlugin] Task ${task.id} assigned to ${matchedAgent} with backend Flowlock.`);
            }
            return {
                success: true,
                agent: matchedAgent,
                task: acceptedTask.id,
                flowlockSession: flowlockResult.session?.id || null
            };
        }

        throw new Error(flowlockResult?.error || 'backend_flowlock_start_failed');
    } catch (error) {
        ...
        if (acceptedTask?.assignment_id) {
            try {
                await pluginManagerRef.processToolCall('TaskBoard', {
                    ...
                    task_id: acceptedTask.id,
                    assignment_id: acceptedTask.assignment_id,
                    reason: `Backend Flowlock 启动失败，回滚分配: ${error.message}`,
...
async function wakeAgentForContinuation(agentName) {
    ...
    try {
        ...
        if (flowlockResult && flowlockResult.status === 'success') {
            if (DEBUG_MODE) {
                console.error(`[AutonomousSchedulerPlugin] Started or resumed Flowlock for ${agentName}, task ${agentTask.id}`);
            }
            return {
                success: true,
                agent: agentName,
        ...
        return { success: false, reason: flowlockResult?.error || 'backend_flowlock_start_failed' };
...
async function handleGetSchedulerStatus() {
    let backendFlowlock = null;
    if (pluginManagerRef) {
        try {
            const result = await pluginManagerRef.processToolCall('BackendFlowlockPlugin', {
                command: 'GetBackendFlowlockStatus'
            });
            if (result?.status === 'success') backendFlowlock = result.backendFlowlock;
        } catch (error) {
            backendFlowlock = { available: false, error: error.message };
        }
    }
    ...
    return {
        ...
        scheduler: {
            ...
            backend_flowlock: backendFlowlock
        }
    };
}
