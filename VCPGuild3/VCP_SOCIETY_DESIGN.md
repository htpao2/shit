...
### 2.4 协作引擎 (The Guild) - 任务日记与后端心流
为了实现真正的协同，任务不再是冷冰冰的状态变更，而是拥有**“任务日记” (Task Diary)**，并由 `BackendFlowlockPlugin` 在后端持续推进。

- 调度器原子接取任务后启动后端 Flowlock，不再直接进行一次性 Agent 通讯。
- 后端 Session 绑定 `agentName + taskId`，同一 Agent 的多个任务可以并行且上下文隔离。
- 每轮通过 AgentAssistant 独立 `session_id` 执行，并重新注入最新任务状态与任务日记。
- 服务重启后仅恢复负责人、状态和 `assignment_id` 仍一致的任务。
...
### 第三阶段：任务日记与后端心流 (`TaskBoardPlugin` + `BackendFlowlockPlugin`)
...
4. **后端循环**: BackendFlowlockPlugin 每轮注入最新任务和日记，通过 AgentAssistant 执行。
5. **持久化恢复**: Session 原子落盘，重启后与任务板对账再恢复。
