...
任务协作由 TaskBoard、任务日记、`TaskFlowlockBridgePlugin` 和 AgentAssistant 官方 Flowlock 共同完成。调度器原子接取任务后，桥接层持久化 `taskId -> delegationId`，实际协议解析、NextPrompt、NextHeartbeat 和模型循环均由 AgentAssistant 异步委托处理。
...
1. **确保插件已加载**
   ...
   - `TaskFlowlockBridgePlugin` - TaskBoard 与 AA delegation 的持久化映射和对账
   - `AgentAssistant` - 官方后端 Flowlock 协议与心跳内核
...
当前 `VCPSocietyScheduler` 的天气、新闻和论坛脉搏仍采用单轮唤醒。第一阶段只有 TaskBoard 任务会通过桥接层创建 AA Flowlock 委托，避免普通社会事件未经明确任务化就形成长期循环。
...
| AgentAssistant 官方 Flowlock | delegationId | AA 内存委托状态 | 后端自主心跳与模型循环 |
| TaskFlowlockBridgePlugin | Agent + Task | 持久化映射 + TaskBoard | 任务所有权、对账与失联重派 |

同一 Agent 的多个 AA 委托当前会串行排队。桥接层不会复制 Flowlock 协议或定时器。
