...
### 3.2 TaskBoard 的配置来源需要实测确认

[`TaskBoardPlugin.js`](../TaskBoardPlugin/TaskBoardPlugin.js:14) 从 `process.env` 读取数据目录、任务超时、并发上限和调试开关，而不是从 `initialize(config)` 读取配置。真实 VCP 是否会把 manifest/config.env 配置注入到 stdio 子进程的环境变量，必须在测试中确认。
...
让 TaskBoard stdio 插件无法启动，再调用桥接或调度器。
