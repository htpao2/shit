...
| `FailTask` | 标记任务失败 |
| `AbandonTask` | 放弃任务 |
| `AppendTaskLog` | **[新增]** 向任务日记追加进度记录 |
| `ReadTaskLog` | **[新增]** 读取任务日记内容 |

#### 任务日记系统 (Task Diary)

为了支持多Agent协作和长周期任务，每个任务都拥有一个独立的“日记本”。

- **存储位置**: `Plugin/TaskBoardPlugin/data/logs/{taskId}.md`
- **功能**:
  - **进度追踪**: Agent在执行任务过程中，必须将关键步骤、思考过程、中间结果写入日记。
  - **上下文共享**: 当新Agent加入任务（或接手任务）时，通过读取日记即可瞬间获得完整的任务上下文，无需口头交接。
  - **自动注入**: 调度器在唤醒Agent执行任务时，会自动读取该任务的日记摘要并注入到Prompt中。
...
调度器只负责发现、匹配和启动，不持有长任务循环。任务匹配成功后，它先通过 TaskBoard 原子接取任务，再调用 `TaskFlowlockBridgePlugin.StartTaskFlowlock`。桥接层以 `task_delegation=true` 提交给 AgentAssistant，由官方 AA Flowlock 内核执行自主心跳。若 AA 在返回 `delegationId` 前启动失败，调度器使用本次 `assignment_id` 补偿释放任务。
...
5. 调用 TaskBoardPlugin.ListTasks
 ...
 7. 选择最合适      │
    的Agent         │
     ...
     └──────┬───────┘
            ▼
 8. TaskBoard 原子接取并生成 assignment_id
            │
            ▼
 9. TaskFlowlockBridgePlugin 持久化 taskId / delegationId 映射
            │
            ▼
10. AgentAssistant 官方 Flowlock 执行心跳，桥接层定期对账
```
...
### 3. TaskFlowlockBridgePlugin (任务心流桥接服务)

**类型**: 混合服务插件 (`hybridservice`)
**协议**: direct
**位置**: `Plugin/TaskFlowlockBridgePlugin/`

官方 AgentAssistant 已原生支持 Flowlock，因此桥接插件不再实现第二套协议解析、模型循环或心跳定时器。它只负责：

- 校验 TaskBoard 的 `in_progress`、负责人和 `assignment_id`。
- 通过 `task_delegation=true` 和 `inject_tools=TaskBoard` 启动官方 AA Flowlock。
- 持久化 `taskId -> delegationId` 映射与审计信息。
- 定期查询 AA 委托和 TaskBoard 状态并对账。
- 服务重启后识别失联的内存委托，在所有权仍有效时限次重派。
- `submitted/completed` 任务完成映射；负责人或分配令牌变化时停止映射。

Flowlock 的唯一后端内核是 `AgentAssistant/flowlockProtocol.js` 与 `AgentAssistant.executeDelegation()`。同一 Agent 的多个委托受 AA 固定 delegation session 锁影响，会串行排队，而不是保证真正并行。

### 4. AgentAssistant (Agent协作插件)
...
4. **小克**调用 `AppendTaskLog` 记录："已创建基础代码框架，正在编写测试用例..."
5. **小克**使用 PowerShellExecutor 运行测试
6. **小克**调用 `AppendTaskLog` 记录："测试通过，API功能正常。"
7. **小克**提交任务成果
8. **小娜**（管理员）验收任务，查阅任务日记确认开发细节
...
4. **小芸**使用设计工具创建UI草图，并调用 `AppendTaskLog` 上传草图链接："UI草图已完成，文件位于..."
5. **小克**读取任务日记，获取草图信息
6. **小克**根据草图编写前端代码，并记录进度
7. 两人协作完成，**小芸**提交任务
...
2. **任务上限**：每个 Agent 同时最多接取3个任务；同一 Agent 的 AA 委托可能串行排队
3. **超时机制**：任务超时由 TaskBoard 释放；AA 委托还受 `DELEGATION_TIMEOUT` 和 `DELEGATION_MAX_ROUNDS` 约束
4. **原子认领**：TaskBoard 使用跨进程写锁及 `assignment_id` 防止重复接取与错误补偿
5. **唯一内核**：仅 AgentAssistant 解析 Flowlock 并执行心跳，桥接层不复制协议状态机
6. **持久化映射**：桥接层原子保存 `taskId`、`assignment_id` 与 `delegationId`
7. **恢复校验**：重启后先检查任务所有权，再查询 AA 结果；失联委托只在重派上限内重新提交
