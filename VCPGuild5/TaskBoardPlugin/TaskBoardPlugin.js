...
// ============== 配置 ==============
let DATA_DIR = process.env.TASK_BOARD_DATA_DIR || path.join(__dirname, 'data');
let TASKS_FILE = path.join(DATA_DIR, 'tasks.json');
let LOGS_DIR = path.join(DATA_DIR, 'logs');
let TASK_LOCK_DIR = path.join(DATA_DIR, '.taskboard-write.lock');
let TASK_LOCK_STALE_MS = parseInt(process.env.TASK_LOCK_STALE_MS || '30000', 10);
let MAX_ACTIVE_TASKS = parseInt(process.env.MAX_ACTIVE_TASKS || '3', 10);
let TASK_TIMEOUT_HOURS = parseInt(process.env.TASK_TIMEOUT_HOURS || '24', 10);
let DEBUG_MODE = String(process.env.DEBUG_MODE || 'false').toLowerCase() === 'true';
let initialized = false;

function initialize(config = {}) {
    DATA_DIR = config.TASK_BOARD_DATA_DIR || process.env.TASK_BOARD_DATA_DIR || path.join(__dirname, 'data');
    if (!path.isAbsolute(DATA_DIR)) DATA_DIR = path.resolve(__dirname, DATA_DIR);
    TASKS_FILE = path.join(DATA_DIR, 'tasks.json');
    LOGS_DIR = path.join(DATA_DIR, 'logs');
    TASK_LOCK_DIR = path.join(DATA_DIR, '.taskboard-write.lock');
    TASK_LOCK_STALE_MS = parseInt(config.TASK_LOCK_STALE_MS || process.env.TASK_LOCK_STALE_MS || '30000', 10);
    MAX_ACTIVE_TASKS = parseInt(config.MAX_ACTIVE_TASKS || process.env.MAX_ACTIVE_TASKS || '3', 10);
    TASK_TIMEOUT_HOURS = parseInt(config.TASK_TIMEOUT_HOURS || process.env.TASK_TIMEOUT_HOURS || '24', 10);
    DEBUG_MODE = String(config.DEBUG_MODE ?? config.DebugMode ?? process.env.DEBUG_MODE ?? 'false').toLowerCase() === 'true';
    ensureDataDir();
    initialized = true;
    const log = config.__stdioCli ? console.error : console.log;
    log(`[TaskBoard] Initialized with data directory: ${DATA_DIR}`);
}

function shutdown() {
    releaseTaskWriteLock();
    initialized = false;
    if (DEBUG_MODE) console.error('[TaskBoard] Shutdown complete.');
}
...
/**
 * 处理 direct/hybridservice 插件调用。返回结构化对象，不经过 stdio 文本包装。
 */
function processToolCall(request = {}) {
    if (!initialized) initialize();
    const command = request.command;
    const maid = request.maid;
    const args = { ...request };
    delete args.command;
    delete args.maid;
...
function main() {
    ...
    process.stdin.on('end', () => {
        try {
            ...
            if (!initialized) initialize({ __stdioCli: true });
...
// 作为独立脚本运行时保留 stdin 兼容；被 PluginManager require 时不启动 stdin 主循环。
