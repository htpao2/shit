...
async function checkAgentBusy(agentName) {
    ...
    try {
        ...
        if (result && result.status === 'success' && Array.isArray(result.tasks)) {
            return result.tasks.some(task => task.assignee === agentName);
        }
    } catch (e) {
        // 忽略错误
    }
...
async function wakeAgent(agentName, monologue) {
    ...
    try {
        ...
        const hasTextContent = Array.isArray(result?.content)
            && result.content.some(part => part?.type === 'text' && String(part.text || '').trim());
        
        if (result && (result.status === 'success' || hasTextContent)) {
            if (DEBUG_MODE) {
