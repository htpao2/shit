...
class PluginManager {
        ...
        try {
            ...
            for (const pluginName of initializationOrder) {
                ...
                try {
                    ...
                    // --- ToolResultRAG 依赖注入 ---
                    if (manifest.name === 'ToolResultRAG') {
                        // 注入 knowledgeBaseManager (来自全局 require)
                        try {
                            const knowledgeBaseManager = require('./KnowledgeBaseManager');
                            dependencies.knowledgeBaseManager = knowledgeBaseManager;
                            if (this.debugMode) console.log(`[PluginManager] Injected knowledgeBaseManager into ToolResultRAG.`);
                        } catch (e) {
                            console.error(`[PluginManager] Failed to inject knowledgeBaseManager into ToolResultRAG:`, e.message);
                        }
                    }
    ...
    async processToolCall(toolName, toolArgs, requestIp = null) {
        ...
        try {
            ...
            // --- 🌟 ToolResultRAG 自动拦截逻辑 ---
            // 检查被调用的插件是否启用了 autoRAG 功能
            const autoRAGConfig = plugin.capabilities?.autoRAG;
            if (autoRAGConfig && autoRAGConfig.enabled) {
                const threshold = autoRAGConfig.threshold || 2000;
                const resultString = JSON.stringify(finalResultObject);
                
                if (resultString.length > threshold) {
                    // 获取 ToolResultRAG 服务模块
                    const toolResultRAGModule = this.getServiceModule('ToolResultRAG');
                    if (toolResultRAGModule && typeof toolResultRAGModule.indexContent === 'function') {
                        try {
                            const ttlMs = autoRAGConfig.ttlMinutes ? autoRAGConfig.ttlMinutes * 60 * 1000 : 30 * 60 * 1000;
                            const resultId = await toolResultRAGModule.indexContent(resultString, {
                                toolName: toolName,
                                ttlMs: ttlMs
                            });
                            ...
                            if (resultId) {
                                ...
                                const simplifiedResult = {
                                    status: 'indexed',
                                    message: `[ToolResultRAG] 工具 ${toolName} 的返回结果过长 (${resultString.length} 字符)，已自动建立索引。请使用 query_tool_result 工具查询具体内容。`,
                                    result_id: resultId,
                                    original_length: resultString.length,
                                    tool_name: toolName,
                                    ttl_minutes: ttlMs / 60000,
                                    hint: `调用示例: query_tool_result(result_id="${resultId}", query="你想查询的内容关键词")`
                                };
                                
                                if (maidNameFromArgs) {
                                    simplifiedResult.MaidName = maidNameFromArgs;
                                }
                                simplifiedResult.timestamp = _getFormattedLocalTimestamp();
                                
                                if (this.debugMode) {
                                    console.log(`[PluginManager] ToolResultRAG intercepted ${toolName} output (${resultString.length} chars -> indexed as ${resultId})`);
                                }
                                
                                return simplifiedResult;
                            }
                        } catch (ragError) {
                            console.error(`[PluginManager] ToolResultRAG indexing failed for ${toolName}:`, ragError.message);
                            // 索引失败时，回退到返回原始结果
                        }
                    } else {
                        if (this.debugMode) {
                            console.warn(`[PluginManager] Plugin ${toolName} has autoRAG enabled, but ToolResultRAG service is not available.`);
                        }
                    }
                }
            }
            // --- ToolResultRAG 拦截逻辑结束 ---
