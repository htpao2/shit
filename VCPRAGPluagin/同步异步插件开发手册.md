...
## 6. ToolResultRAG：工具结果的 RAG 检索增强

当某些工具（如网页爬虫、文件搜索、数据库查询等）返回过于冗长的结果时，直接将完整结果传递给 AI 会导致：
1.  **Token 消耗过高**：大量无关信息占用上下文窗口。
2.  **响应速度下降**：AI 处理长文本需要更多时间。
3.  **信息噪声**：AI 可能难以从海量数据中提取关键信息。

ToolResultRAG 是一个内置的混合服务插件，它通过 **自动拦截** 过长的工具输出，将其建立临时向量索引，并提供 `query_tool_result` 工具让 AI 按需检索相关片段。
...
└─────────────────────────────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│  1. PluginManager.processToolCall() 执行目标插件                 │
│     获取原始返回结果 (可能超过 2000 字符)                          │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│  2. 检查目标插件的 manifest 是否有 capabilities.autoRAG           │
│     - enabled: true                                             │
│     - threshold: 2000 (字符阈值)                                 │
│     - ttlMinutes: 30 (索引存活时间)                              │
└─────────────────────────────────────────────────────────────────┘
                                │
                    ┌───────────┴───────────┐
                    │                       │
...
要让你的插件支持自动 RAG 拦截，只需在 `plugin-manifest.json` 中添加 `capabilities.autoRAG` 配置：

```json
{
  "name": "MyLongOutputPlugin",
  "displayName": "可能返回长结果的插件",
  "pluginType": "synchronous",
  "entryPoint": {
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | boolean | `false` | 是否启用自动 RAG 拦截 |
| `threshold` | number | `2000` | 触发拦截的字符数阈值。当 `JSON.stringify(result).length > threshold` 时触发 |
| `ttlMinutes` | number | `30` | 临时索引的存活时间（分钟）。过期后索引自动销毁 |

### 6.3 AI 如何使用 query_tool_result

当工具返回被拦截后，AI 会收到类似以下的响应：

```json
{
  "status": "indexed",
  "message": "[ToolResultRAG] 工具 WebCrawler 的返回结果过长 (15678 字符)，已自动建立索引。请使用 query_tool_result 工具查询具体内容。",
  "result_id": "res_a1b2c3d4-e5f6-7890-abcd-ef1234567890",
  "original_length": 15678,
  "tool_name": "WebCrawler",
  "ttl_minutes": 30,
  "hint": "调用示例: query_tool_result(result_id=\"res_a1b2c3d4...\", query=\"你想查询的内容关键词\")"
}
```

AI 随后可以调用 `query_tool_result` 来检索相关信息：

```
<<<[TOOL_REQUEST]>>>
tool_name:「始」ToolResultRAG「末」,
result_id:「始」res_a1b2c3d4-e5f6-7890-abcd-ef1234567890「末」,
query:「始」产品价格「末」,
top_k:「始」5「末」
<<<[END_TOOL_REQUEST]>>>
```
...
```json
{
  ...
  "results": [
    {
      "text": "产品A的市场价格为 ¥299，促销价 ¥249...",
      "score": 0.92,
      "source": "Transient Tool Result"
    },
    {
      "text": "高端版本定价 ¥599，包含额外配件...",
      "score": 0.87,
      "source": "Transient Tool Result"
    }
    // ... 更多结果
...
以下类型的插件特别适合启用 AutoRAG：

| 插件类型 | 示例 | 为什么需要 |
|----------|------|------------|
| 网页爬虫 | `WebCrawler`, `TavilySearch` | 网页内容往往包含大量无关的导航、广告文本 |
| 文件搜索 | `FileSearcher` | 搜索结果可能包含数十个匹配项 |
| 数据库查询 | `SQLExecutor` | 查询结果可能返回大量行数据 |
| API 调用 | `GitHubAPI` | API 响应（如仓库列表、Issue 列表）可能非常长 |
| 日志分析 | `LogAnalyzer` | 日志文件通常包含大量条目 |

### 6.5 注意事项

1.  **不要对所有插件启用**：对于返回结构化、简短数据的插件（如计算器、天气查询），启用 AutoRAG 反而会增加不必要的复杂度。

2.  **合理设置阈值**：
    - 阈值太低（如 500）：可能导致正常长度的结果也被拦截，增加 AI 调用次数。
    - 阈值太高（如 10000）：可能无法有效减少 Token 消耗。
    - 建议值：2000-5000 字符。

3.  **TTL 设置**：
    - 如果任务需要多轮对话深入分析，可设置较长的 TTL（如 60 分钟）。
    - 如果是一次性查询，可设置较短的 TTL（如 10 分钟）以节省内存。

4.  **向量化成本**：每次建立索引都需要调用 Embedding API，会产生一定的 API 成本。确保你的 Embedding 配置正确（`API_Key`, `API_URL`, `WhitelistEmbeddingModel`）。

### 6.6 技术实现细节

ToolResultRAG 基于以下核心组件：

- **[`KnowledgeBaseManager.js`](KnowledgeBaseManager.js)**: 提供 `addTransientContent()` 和 `searchTransientContent()` 方法，管理临时向量索引。
- **[`TextChunker.js`](TextChunker.js)**: 将长文本分割成适合向量化的块。
- **[`EmbeddingUtils.js`](EmbeddingUtils.js)**: 调用 Embedding API 将文本转换为向量。
- **[`rust-vexus-lite`](rust-vexus-lite)**: 高性能的内存向量索引引擎。

**临时索引存储结构：**

```javascript
// KnowledgeBaseManager.transientIndices
Map<resultId, {
  index: VexusIndex,       // Vexus 向量索引实例
  chunks: Array<string>,   // 原始文本块（用于返回搜索结果）
  timer: Timeout,          // TTL 定时器
  createdAt: number        // 创建时间戳
}>
```

当 TTL 到期时，`_destroyTransientIndex()` 方法会自动清理索引和相关数据，释放内存。
