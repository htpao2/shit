...
def get_desktop_info():  
    try:  
        ...
        content_list = []  
        details_dict = state_dict.copy()  
  
        if 'screenshot' in details_dict and isinstance(details_dict['screenshot'], bytes):  
            screenshot_base64 = base64.b64encode(details_dict['screenshot']).decode('utf-8')  
            content_list.append({  
                'type': 'image_url',  
                'image_url': {  
            ...
            del details_dict['screenshot']  
            details_dict['screenshot_base64'] = screenshot_base64  
        ...
        return {"status": "error", "error": f"Failed to get desktop info: {type(e).__name__} - {str(e)}"}
        ...
        try:
            ...
            if ps_proc.returncode == 0:
                ...
                if cmd_proc.returncode == 0:
                    ...
                    return {"status": "error", "error": f"Failed to launch {name}. {detail}"}  
        except Exception as sub_e:
            return {"status": "error", "error": f"Failed to launch {name} with fallback: {type(sub_e).__name__} - {str(sub_e)}"}


# powershell_tool 已移除 - 用户可使用其他插件执行PowerShell命令
...
def click_tool(loc: tuple, button: str = 'left', clicks: int = 1):
    if not loc or len(loc) != 2:
        return {"status": "error", "result": "Invalid location parameter. Expected tuple of (x, y)."}
    
    try:
        x, y = loc
        # 统一使用 pyautogui 进行鼠标控制
        ...
        return {"status": "success", "result": f'{num_clicks.get(clicks, "Multiple")} {button} clicked on {control.Name} Element with ControlType {control.ControlTypeName} at ({x},{y}).'}
    except Exception as e:
        return {"status": "error", "result": f"Click failed: {type(e).__name__} - {str(e)}"}

def type_tool(text: str, loc: tuple = None, clear: bool = False):
    if text is None:
        return {"status": "error", "result": "Missing 'text' parameter for type_tool."}
    ...
    try:
        ...
        return {"status": "error", "result": f"Type failed: {type(e).__name__} - {str(e)}"}
...
def scroll_tool(direction: Literal['up', 'down'], amount: int = 0):
    try:
        if direction == 'up':
            ...
            return {"status": "error", "result": "Invalid direction. Use 'up' or 'down'."}
        return {"status": "success", "result": f'Scrolled {direction} by {amount}.'}
    except Exception as e:
        return {"status": "error", "result": f"Scroll failed: {type(e).__name__} - {str(e)}"}

def drag_tool(from_loc: tuple, to_loc: tuple):
    if not from_loc or len(from_loc) != 2 or not to_loc or len(to_loc) != 2:
        return {"status": "error", "result": "Invalid location parameters. Expected tuples of (x, y)."}
    ...
    try:
        ...
        return {"status": "error", "result": f"Drag failed: {type(e).__name__} - {str(e)}"}

def move_tool(to_loc: tuple = (0, 0)):
    if not to_loc or len(to_loc) != 2:
        return {"status": "error", "result": "Invalid location parameter. Expected tuple of (x, y)."}
    
    try:
        x, y = to_loc
        # 统一使用 pyautogui 进行鼠标移动
        pg.moveTo(x, y)
        return {"status": "success", "result": f'Moved the mouse pointer to ({x},{y}).'}
    except Exception as e:
        return {"status": "error", "result": f"Move failed: {type(e).__name__} - {str(e)}"}
