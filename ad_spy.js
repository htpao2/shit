...
function getTransparencyUrl(domain, region = null) {
    ...
    let url = 'https://adstransparency.google.com/';
    const params = [];
    ...
    params.push(`domain=${encodeURIComponent(cleanDomain)}`);
...
function getTransparencyUrlCommand(domain) {
    ...
    const resultParts = [
        ...
        `4. 可查看广告的投放历史\n`,
        `### 快捷链接\n`,
        `- [全球广告](${getTransparencyUrl(domain)})`,
        `- [美国地区](${getTransparencyUrl(domain, 'US')})`,
        `- [英国地区](${getTransparencyUrl(domain, 'GB')})`,
        `- [日本地区](${getTransparencyUrl(domain, 'JP')})`,
        `- [中国大陆](${getTransparencyUrl(domain, 'CN')}) *(可能无数据)*`
    ];
