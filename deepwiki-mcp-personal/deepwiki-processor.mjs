...
/**
 * DeepwikiProcessor VCP同步插件 (ES模块版本)
 * 
 * 基于原有deepwiki-mcp项目改造的VCP插件
 * 支持从deepwiki.com爬取内容并转换为Markdown格式
 * 支持搜索功能和文件保存功能
 */
...
// 动态导入核心模块
let crawl, htmlToMarkdown, resolveRepo, extractKeyword, FetchRequest;

async function initializeModules() {
  try {
    // 尝试从dist目录导入编译后的模块
    const crawlerModule = await import('./dist/lib/httpCrawler.mjs').catch(() =>
      import('./src/lib/httpCrawler.js')
    );
    const converterModule = await import('./dist/converter/htmlToMarkdown.mjs').catch(() =>
      import('./src/converter/htmlToMarkdown.js')
    );
    const resolveModule = await import('./dist/utils/resolveRepoFetch.mjs').catch(() =>
      import('./src/utils/resolveRepoFetch.js')
    );
    const extractModule = await import('./dist/utils/extractKeyword.mjs').catch(() =>
      import('./src/utils/extractKeyword.js')
    );
    const schemaModule = await import('./dist/schemas/deepwiki.mjs').catch(() =>
      import('./src/schemas/deepwiki.js')
    );
    ...
    FetchRequest = schemaModule.FetchRequest;
  } catch (error) {
...
async function normalizeUrl(url) {
  ...
  if (/^[^/]+$/.test(normalizedUrl)) {
    ...
    try {
      ...
      // 回退到默认处理
      return `https://deepwiki.com/defaultuser/${normalizedUrl}`;
    }
  }
  ...
  if (extracted) {
    try {
      ...
      return `https://deepwiki.com/defaultuser/${extracted}`;
    }
  }
...
function parseBatchRequest(request) {
  ...
  while (request[`command${index}`]) {
    const command = {
      ...
      maxDepth: request[`maxDepth${index}`],
      saveFormat: request[`saveFormat${index}`],
      outputPath: request[`outputPath${index}`],
      verbose: request[`verbose${index}`],
      query: request[`query${index}`],
      maxMatches: request[`maxMatches${index}`],
      saveResults: request[`saveResults${index}`]
    };
...
async function main() {
  try {
    ...
    // 解析批量请求
    const commands = parseBatchRequest(request);
    const results = [];
    ...
    for (const cmd of commands) {
      ...
      switch (cmd.command) {
        case 'FetchContent':
          result = await handleFetchContent(cmd);
          break;
        case 'SearchContent':
          result = await handleSearchContent(cmd);
          break;
        default:
