...
import { htmlToMarkdown } from './converter/htmlToMarkdown.mjs';
import { resolveRepo } from './utils/resolveRepoFetch.mjs';
import { extractKeyword } from './utils/extractKeyword.mjs';
import { crawl } from './lib/httpCrawler.mjs';
import { FetchRequest } from './schemas/deepwiki.mjs';
import 'linkedom';
import 'rehype-parse';
...
import 'unified';
import './lib/linkRewrite.mjs';
import 'unist-util-visit';
import './lib/sanitizeSchema.mjs';
...
async function startServer(server, options = { type: "stdio" }) {
  ...
  if (options.type === "http") {
    const port2 = options.port ?? 3e3;
    const endpoint = options.endpoint ?? "/mcp";
    const transport = new RestServerTransport({ port: port2, endpoint });
    await server.connect(transport);
    await transport.startServer();
    console.log(`HTTP server listening \u2192 http://localhost:${port2}${endpoint}`);
    return;
  }
  ...
  app.use(router);
  const nodeServer = createServer$1(toNodeListener(app));
