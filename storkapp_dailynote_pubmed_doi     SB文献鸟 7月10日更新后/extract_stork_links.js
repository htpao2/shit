...
/**
 * 从给定的文本内容中提取 PubMed ID 和可选 DOI。
 * 兼容旧格式 "PMID: 123 doi: 10.x/x" 和新格式
 * "PMID 123 · doi 10.x/x"，也保留只有 PMID 的记录。
 * @param {string} content - 文件内容。
 * @returns {Array<{pmid: string, doi: string}>} - 提取到的论文信息数组
 */
function extractPaperInfo(content) {
    const papers = [];
    const regex = /\bPMID\s*:?\s*(\d{5,})(?:\s*(?:·|[|;,])?\s*doi\s*:?\s*([^\s<>"')]+))?/gi;
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            try {
                ...
                extracted.forEach(p => {
                    const existingDoi = uniquePapers.get(p.pmid) || '';
                    if (!uniquePapers.has(p.pmid) || (!existingDoi && p.doi)) {
