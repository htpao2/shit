...
async function fetchSimilarPubmedIds(pubmedId, storkid) {
  // 强制停用文献鸟相似 API，直接返回空以触发 NCBI ELink 回退
  return [];
}
...
/**
 * 批量 EFetch 解析 DOI。
 * @returns {Promise<{doiMap: Map<string, string>, failedIds: Set<string>}>}
 */
async function efetchDoisForPubmedIds(pubmedIdsIterable) {
  ...
  function parseXmlAndFill(xmlText) {
    ...
    for (const art of articles) {
      const citationMatch = art.match(/<MedlineCitation[\s\S]*?<PMID[^>]*>(\d+)<\/PMID>/i);
      if (!citationMatch) continue;

      const pmid = citationMatch[1];
      // 只读取主记录 PubmedData 下的 ArticleIdList，避免误取参考文献 DOI。
      const pubmedDataMatch = art.match(/<PubmedData[\s\S]*?<ArticleIdList>([\s\S]*?)<\/ArticleIdList>[\s\S]*?<\/PubmedData>/i);
      const articleIds = pubmedDataMatch ? pubmedDataMatch[1] : '';
      const doiMatch = articleIds.match(/<ArticleId[^>]*IdType="doi"[^>]*>([^<]+)<\/ArticleId>/i);
      if (doiMatch && doiMatch[1]) {
        doiMap.set(pmid, doiMatch[1].trim());
      } else if (!doiMap.has(pmid)) {
        doiMap.set(pmid, '');
      }
    }
  }
    ...
    for (let attempt = 0; attempt < NCBI_EUTILS_MAX_RETRIES; attempt++) {
      try {
        ...
        for (const pmid of chunk) {
          if (!doiMap.has(pmid)) doiMap.set(pmid, '');
        }
        success = true;
        break;
      } catch (err) {
...
async function main() {
  ...
  // 检查输入格式。如果是旧的 storkid（只有数字），则需要 fetchShowPaperHtml。
  // 如果是新的 PMID\tDOI 格式，则直接使用。
  // 但是 readStorkIds 已经通过 readIds 返回了行。
  const inputLines = await readIds(fssync.existsSync(NEW_STORK_TMP) ? NEW_STORK_TMP : STORK_IDS_TMP);

  for (let i = 0; i < inputLines.length; i++) {
    const line = inputLines[i];
    const parts = line.split('\t');
    let pmid = parts[0].trim();
    let doi = parts[1] ? parts[1].trim() : '';

    console.log(`[${i + 1}/${inputLines.length}] 处理 PMID=${pmid}`);

    storkDoiMap.set(pmid, doi);

    // 调用相似接口。注意：related_api.php 接收的是 pubmedId
    const primarySimilars = await fetchSimilarPubmedIds(pmid, pmid);
    let finalSimilars = primarySimilars;
    let usedFallback = false;

    if (finalSimilars.length === 0) {
      console.log('  - 相似列表为空，尝试 NCBI ELink 备用来源...');
      finalSimilars = await fetchSimilarPubmedIdsViaNCBI(pmid);
  ...
  const mainPmidsMissingDoi = storkIds.filter(pmid => !(storkDoiMap.get(pmid) || '').trim());
  if (FETCH_PUBMED_DOI && mainPmidsMissingDoi.length > 0) {
    console.log(`[INFO] 开始通过 NCBI EFetch 补查 ${mainPmidsMissingDoi.length} 条主文献 DOI（批次=${NCBI_EFETCH_BATCH}, interval=${NCBI_EUTILS_INTERVAL_MS}ms）`);
    const mainLookup = await efetchDoisForPubmedIds(mainPmidsMissingDoi);
    for (const pmid of mainPmidsMissingDoi) {
      const doi = (mainLookup.doiMap.get(pmid) || '').trim();
      if (doi) {
        storkDoiMap.set(pmid, doi);
        console.log(`[OK] 主文献 DOI 补查成功: PMID=${pmid}, DOI=${doi}`);
      } else if (mainLookup.failedIds.has(pmid)) {
        console.warn(`[WARN] 主文献 DOI 补查请求失败，跳过: PMID=${pmid}`);
      } else {
        console.warn(`[WARN] NCBI 未提供该主文献 DOI，跳过: PMID=${pmid}`);
      }
    }
  } else if (!FETCH_PUBMED_DOI && mainPmidsMissingDoi.length > 0) {
    console.warn(`[WARN] FETCH_PUBMED_DOI 已关闭，${mainPmidsMissingDoi.length} 条主文献无法补查 DOI。`);
  }
  ...
  if (FETCH_PUBMED_DOI && resultSet.size > 0) {
    ...
    const similarLookup = await efetchDoisForPubmedIds(resultSet);
    pubmedDoiMap = similarLookup.doiMap;
    if (similarLookup.failedIds.size > 0) {
      console.warn(`[WARN] 相似文献 DOI 补查请求失败 ${similarLookup.failedIds.size} 条: ${Array.from(similarLookup.failedIds).join(', ')}`);
    }
  }
...
module.exports = { efetchDoisForPubmedIds };
