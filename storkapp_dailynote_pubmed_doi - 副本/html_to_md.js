...
/**
 * 预处理 HTML，提取并清理正文
 * 新增逻辑：如果识别出是 NCBI XML，则按照 XML 结构提取主要字段
 */
function preprocessHtml(content) {
  // 简单的启发式判断：是否为 NCBI XML
  if (content.includes('<PubmedArticle') || content.includes('<PubmedArticleSet')) {
    const titleMatch = content.match(/<ArticleTitle>(.*?)<\/ArticleTitle>/i);
    const abstractMatch = content.match(/<AbstractText.*?>(.*?)<\/AbstractText>/gi);
    const journalMatch = content.match(/<Title>(.*?)<\/Title>/i); // 刊名
    const yearMatch = content.match(/<Year>(\d{4})<\/Year>/i);
    const keywordMatch = content.match(/<Keyword.*?>(.*?)<\/Keyword>/gi);
    ...
    let abstract = '';
    if (abstractMatch) {
       abstract = abstractMatch.map(m => m.replace(/<AbstractText.*?>|<\/AbstractText>/gi, '').trim()).join('\n\n');
    }

    let tags = '';
    if (keywordMatch) {
       const keywords = keywordMatch.map(m => m.replace(/<Keyword.*?>|<\/Keyword>/gi, '').trim()).filter(Boolean);
       if (keywords.length > 0) {
         tags = '\n\nTag: ' + keywords.join(', ');
       }
    }

    return `<h1>${title}</h1><p><i>${journal} (${year})</i></p><div>${abstract}</div>${tags}`;
  }
