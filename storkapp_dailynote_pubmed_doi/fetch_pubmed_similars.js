...
async function main() {
  ...
  for (let i = 0; i < inputLines.length; i++) {
    ...
    // 如果没有 DOI（可能是旧数据或提取失败），尝试通过 storkapp.cn 补全（但通常域名炸了会失败）
    if (!doi && pmid.length < 15) { // 简单的启发式判断：storkid 通常较短？不，其实不需要判断，炸了就是炸了。
        // 原逻辑 fetchShowPaperHtml(pmid) 实际上是把 pmid 当做 storkid 传。
        // 这里我们优先相信 PMID。
    }
